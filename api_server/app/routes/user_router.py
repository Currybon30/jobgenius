...
from app.services.user_service import (add_user_to_db, get_current_user,
                                       get_user_by_id, update_user_plan)
...
def update_user_subscription_plan(user_id: int, db: Annotated[Session, Depends(get_db)], x_api_key: Annotated[str, Header(...)], data: UserPlanUpdate):
    verify_api_key(x_api_key)
    logger.info(
        f"Received request to update plan for user {user_id} with data: {data}")
